...
**Acceptance Criteria:**
- [] 정규식으로 HTML에서 링크(`<a>`), 폼(`<form>`, `<input>`), 버튼(`<button>`), 스크립트 내 API 호출 패턴을 추출한다
- [] 크롤링 결과를 구조화된 사이트맵으로 저장한다
- [] 크롤링 깊이 및 범위를 사용자가 설정할 수 있다
- [] robots.txt 준수 옵션을 제공한다
- [] LLM API 없이 오프라인 환경에서도 동작한다
- [] URL, 파라미터, 엔드포인트 목록을 JSON 형태로 출력한다
...
- [] JavaScript 렌더링이 필요한 SPA(Single Page Application)를 지원한다
