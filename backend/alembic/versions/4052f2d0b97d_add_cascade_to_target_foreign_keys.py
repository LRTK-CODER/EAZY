...
def upgrade() -> None:
    ...
    op.drop_constraint('asset_discoveries_task_id_fkey', 'asset_discoveries', type_='foreignkey')
    op.create_foreign_key(
        'asset_discoveries_task_id_fkey',
        'asset_discoveries', 'tasks',
        ['task_id'], ['id'],
        ondelete='CASCADE'
    )

    # 4. asset_discoveries.asset_id FK 재생성 (CASCADE 추가)
    op.drop_constraint('asset_discoveries_asset_id_fkey', 'asset_discoveries', type_='foreignkey')
    op.create_foreign_key(
        'asset_discoveries_asset_id_fkey',
        'asset_discoveries', 'assets',
        ['asset_id'], ['id'],
        ondelete='CASCADE'
    )

    # 5. asset_discoveries.parent_asset_id FK 재생성 (CASCADE 추가)
    op.drop_constraint('asset_discoveries_parent_asset_id_fkey', 'asset_discoveries', type_='foreignkey')
    op.create_foreign_key(
...
def downgrade() -> None:
    ...
    op.drop_constraint('tasks_target_id_fkey', 'tasks', type_='foreignkey')
    op.create_foreign_key('tasks_target_id_fkey', 'tasks', 'targets', ['target_id'], ['id'])

    # 2. assets.target_id FK 복원 (ondelete 제거)
    op.drop_constraint('assets_target_id_fkey', 'assets', type_='foreignkey')
    op.create_foreign_key('assets_target_id_fkey', 'assets', 'targets', ['target_id'], ['id'])

    # 3. asset_discoveries.task_id FK 복원 (ondelete 제거)
    op.drop_constraint('asset_discoveries_task_id_fkey', 'asset_discoveries', type_='foreignkey')
    op.create_foreign_key('asset_discoveries_task_id_fkey', 'asset_discoveries', 'tasks', ['task_id'], ['id'])

    # 4. asset_discoveries.asset_id FK 복원 (ondelete 제거)
    op.drop_constraint('asset_discoveries_asset_id_fkey', 'asset_discoveries', type_='foreignkey')
    op.create_foreign_key('asset_discoveries_asset_id_fkey', 'asset_discoveries', 'assets', ['asset_id'], ['id'])

    # 5. asset_discoveries.parent_asset_id FK 복원 (ondelete 제거)
    op.drop_constraint('asset_discoveries_parent_asset_id_fkey', 'asset_discoveries', type_='foreignkey')
    op.create_foreign_key('asset_discoveries_parent_asset_id_fkey', 'asset_discoveries', 'assets', ['parent_asset_id'], ['id'])
