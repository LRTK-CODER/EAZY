...
async def test_tasks_target_id_cascade_constraint(db_session: AsyncSession):
    ...
    assert row.delete_rule == 'CASCADE', f"tasks.target_id FK does not have CASCADE (current: {row.delete_rule})"
...
async def test_assets_target_id_cascade_constraint(db_session: AsyncSession):
    ...
    assert row.delete_rule == 'CASCADE', f"assets.target_id FK does not have CASCADE (current: {row.delete_rule})"
...
async def test_asset_discoveries_task_id_cascade_constraint(db_session: AsyncSession):
    ...
    assert row.delete_rule == 'CASCADE', f"asset_discoveries.task_id FK does not have CASCADE (current: {row.delete_rule})"
...
async def test_asset_discoveries_asset_id_cascade_constraint(db_session: AsyncSession):
    ...
    assert row.delete_rule == 'CASCADE', f"asset_discoveries.asset_id FK does not have CASCADE (current: {row.delete_rule})"


@pytest.mark.asyncio
async def test_asset_discoveries_parent_asset_id_cascade_constraint(db_session: AsyncSession):
    ...
    assert row.delete_rule == 'CASCADE', f"asset_discoveries.parent_asset_id FK does not have CASCADE (current: {row.delete_rule})"
