...
async def test_worker_process_task(db_session, redis_client):
    ...
    target = Target(name="Worker Target", project_id=project.id, url="http://example.com", scope=TargetScope.DOMAIN)
    db_session.add(target)
    await db_session.commit()
    await db_session.refresh(target)
    
    task = Task(project_id=project.id, target_id=target.id, type=TaskType.CRAWL, status=TaskStatus.PENDING)
    db_session.add(task)
    await db_session.commit()
    ...
    with unittest.mock.patch.object(CrawlerService, 'crawl', new_callable=unittest.mock.AsyncMock) as mock_crawl:
