...
async def test_read_project(client):
    ...
    create_res = await client.post(
        "/api/v1/projects/", json={"name": "Single Project", "description": "Desc"}
    )
...
async def test_update_project(client):
    ...
    # Create project
    create_res = await client.post(
        "/api/v1/projects/", json={"name": "Original Name", "description": "Original"}
    )
    project_id = create_res.json()["id"]
...
async def test_archive_project(client):
    ...
    # Create project
    create_res = await client.post(
        "/api/v1/projects/", json={"name": "To Archive", "description": ""}
    )
    project_id = create_res.json()["id"]
...
async def test_delete_project_permanent(client):
    ...
    create_res = await client.post(
        "/api/v1/projects/", json={"name": "To Delete Permanently", "description": ""}
    )
    project_id = create_res.json()["id"]
...
async def test_restore_project(client):
    ...
    create_res = await client.post(
        "/api/v1/projects/", json={"name": "To Restore", "description": ""}
    )
    project_id = create_res.json()["id"]
    await client.delete(f"/api/v1/projects/{project_id}")
