...
async def test_read_targets(client):
    ...
    # Create project
    proj_res = await client.post(
        "/api/v1/projects/", json={"name": "List Project", "description": "Desc"}
    )
    project_id = proj_res.json()["id"]
...
async def test_get_target_assets_endpoint_exists(client: AsyncClient):
    ...
    response = await client.get(
        f"/api/v1/projects/{project_id}/targets/{target_id}/assets"
    )

    # Assertion: Endpoint should exist (not 404)
    assert (
        response.status_code != 404
    ), f"Expected endpoint to exist, got status {response.status_code}"
...
async def test_get_target_assets_success(client: AsyncClient, db_session: AsyncSession):
    ...
    # Setup: Create Project + Target
    proj_res = await client.post(
        "/api/v1/projects/", json={"name": "Success Test Project"}
    )
    project_id = proj_res.json()["id"]
    ...
    for asset_data in assets_data:
        content_hash = hashlib.sha256(
            f"{asset_data['method']}:{asset_data['url']}".encode()
        ).hexdigest()
        asset = Asset(
            target_id=target_id,
...
async def test_get_target_assets_target_not_found(client: AsyncClient):
    ...
    proj_res = await client.post(
        "/api/v1/projects/", json={"name": "Project Without Target"}
    )
    project_id = proj_res.json()["id"]
...
@pytest.mark.asyncio
async def test_get_target_assets_cross_project_isolation(
    client: AsyncClient, db_session: AsyncSession
):
    """Test 5: Verify data isolation between projects (authorization check)."""
    # Setup: Create Project A → Target A1 → Asset A1
    proj_a_res = await client.post("/api/v1/projects/", json={"name": "Project A"})
    ...
    proj_b_res = await client.post("/api/v1/projects/", json={"name": "Project B"})
    ...
    response_cross = await client.get(
        f"/api/v1/projects/{project_a_id}/targets/{target_b1_id}/assets"
    )

    # Assertion 1: Should return 404 (Target B1 does not belong to Project A)
    assert (
        response_cross.status_code == 404
    ), f"Expected 404 for cross-project access, got {response_cross.status_code}"

    # Execution 2: Valid access (Project A accessing Target A1)
    response_valid = await client.get(
        f"/api/v1/projects/{project_a_id}/targets/{target_a1_id}/assets"
    )
    ...
    assert (
        asset_b1.id not in asset_ids
    ), "Asset B1 should not be accessible from Project A"
...
async def test_get_target_assets_empty_list(client: AsyncClient):
    ...
    proj_res = await client.post(
        "/api/v1/projects/", json={"name": "Empty Assets Project"}
    )
    project_id = proj_res.json()["id"]
...
@pytest.mark.asyncio
async def test_get_target_assets_deduplication(
    client: AsyncClient, db_session: AsyncSession
):
    """Test 7: Verify content_hash based deduplication (same asset discovered multiple times)."""
    # Setup: Create Project + Target
    proj_res = await client.post(
        "/api/v1/projects/", json={"name": "Deduplication Test Project"}
    )
    project_id = proj_res.json()["id"]
    ...
    assert (
        len(data) == 2
    ), f"Expected 2 unique assets (deduplication applied), got {len(data)}"

    # Verify Asset 1 appears only once
    asset_ids = [asset_json["id"] for asset_json in data]
    assert (
        asset_ids.count(asset1.id) == 1
    ), "Asset 1 should appear only once despite multiple discoveries"
    ...
    assert (
        len(asset1_discoveries) == 2
    ), f"Asset 1 should have 2 discoveries, got {len(asset1_discoveries)}"
    ...
    assert (
        len(asset2_discoveries) == 1
    ), f"Asset 2 should have 1 discovery, got {len(asset2_discoveries)}"


@pytest.mark.asyncio
async def test_get_target_assets_sorted_by_last_seen_desc(
    client: AsyncClient, db_session: AsyncSession
):
    """Test 8: Verify assets are sorted by last_seen_at in descending order (newest first)."""
    # Setup: Create Project + Target
    proj_res = await client.post(
        "/api/v1/projects/", json={"name": "Sorting Test Project"}
    )
    project_id = proj_res.json()["id"]
    ...
    assert (
        data[0]["id"] == asset3.id
    ), f"First asset should be Asset 3 (newest), got {data[0]['id']}"
    assert (
        data[1]["id"] == asset2.id
    ), f"Second asset should be Asset 2 (middle), got {data[1]['id']}"
    assert (
        data[2]["id"] == asset1.id
    ), f"Third asset should be Asset 1 (oldest), got {data[2]['id']}"
