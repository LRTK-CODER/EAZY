...
async def test_trigger_scan_task(client: AsyncClient, db_session: AsyncSession):
    ...
    resp = await client.post(f"/api/v1/projects/{project_id}/targets/", json={
        "name": "Scan Target",
...
async def test_get_task_status(client: AsyncClient, db_session: AsyncSession):
    ...
    assert resp.status_code == 201
    project_id = resp.json()["id"]
    resp = await client.post(f"/api/v1/projects/{project_id}/targets/", json={"name": "Target Status", "url": "http://example.com"})
    assert resp.status_code == 201
    target_id = resp.json()["id"]
    
    resp = await client.post(f"/api/v1/projects/{project_id}/targets/{target_id}/scan")
    if resp.status_code != 202:
        print(f"DEBUG: Scan trigger failed. Status: {resp.status_code}, Body: {resp.text}")
    assert resp.status_code == 202
    ...
    assert data["status"] in [TaskStatus.PENDING, TaskStatus.RUNNING, TaskStatus.COMPLETED, TaskStatus.FAILED]
...
async def test_get_task_assets(client: AsyncClient, db_session: AsyncSession):
    ...
    project_id = resp.json()["id"]
    resp = await client.post(f"/api/v1/projects/{project_id}/targets/", json={"name": "T", "url": "http://example.com"})
    target_id = resp.json()["id"]
    resp = await client.post(f"/api/v1/projects/{project_id}/targets/{target_id}/scan")
    task_id = resp.json()["task_id"]
