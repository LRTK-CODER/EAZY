...
@pytest.mark.asyncio
async def test_delete_target_with_scan_history(client: AsyncClient, db_session: AsyncSession):
    """스캔 이력이 있는 Target 삭제 시 CASCADE 동작 검증 (E2E)"""
    # 1. Create Project & Target
    p_resp = await client.post("/api/v1/projects/", json={"name": "Scan Proj"})
    p_id = p_resp.json()["id"]

    t_resp = await client.post(f"/api/v1/projects/{p_id}/targets/", json={
        "name": "Scan Target",
    ...
    assert task_resp_after.status_code == 404, "Task should be CASCADE deleted with Target"
